...
export function Home() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background pt-20 text-foreground">
        ...
        <div className="max-w-xl">
          ...
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            ...
            <Button
              variant="outline"
              onClick={() =>
                (window.location.href = "mailto:contact@gwilldan.xyz")
              }
          ...
          <p className="mt-5 text-xs text-muted-foreground">
            No spreadsheets. No finance degree.
          </p>
        </div>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 lg:grid-cols-3 lg:px-10 lg:py-20">
          {features.map(({ icon: Icon, title, body }, index) => (
            <div key={title} className="flex gap-4">
              <span
                className={`flex size-10 shrink-0 items-center justify-center rounded-xl ${index === 0 ? "bg-cyan-accent/10" : index === 1 ? "bg-gold-accent/15" : "bg-muted"}`}
              >
                <Icon
                  className={`size-5 ${index === 0 ? "text-cyan-accent" : index === 1 ? "text-gold-accent" : "text-foreground"}`}
                />
              </span>
              <div>
      ...
      <section
        id="how-it-works"
        className="mx-auto flex max-w-7xl flex-col gap-8 px-6 py-20 lg:flex-row lg:items-end lg:justify-between lg:px-10"
      >
        <div>
          ...
          <h2 className="mt-3 max-w-2xl text-4xl font-semibold tracking-[-0.04em] sm:text-5xl">
            From raw transactions to a plan you can act on.
          </h2>
        </div>
        <p className="max-w-sm text-sm leading-6 text-muted-foreground">
          Connect your accounts or upload a statement. ezFinance categorizes
          your activity, spots patterns, and helps you decide what to do next.
        </p>
      </section>
